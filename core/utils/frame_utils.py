...
def readDispFSD(filename):
    disp = depth_uint8_decoding(imageio.imread(filename))
    return disp
...
def readDispStereoCarla(filename):
    f_pix = 1385.64
    baseline = filename[1]
    depth = np.array(Image.open(filename[0]), dtype=np.float32)  # cm
    left_disp = baseline * f_pix / (depth + 1e-6)
    valid = (left_disp > 0) & (left_disp < 512)
    left_disp[~valid] = 0.0
    return left_disp, valid


def readDispDynamicReplica(filename):
    depth_eps = 1e-5
    depth2disp_scale = filename[1]
    with Image.open(filename[0]) as depth_pil:
    ...
    disp[depth_mask] = 0
    valid_disp = (disp > 0) * (1 - depth_mask)
    return disp, valid_disp
